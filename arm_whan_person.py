...
print("Система запущена. Очікування людини в кадрі...")
...
while True:
    ...
    # 3. Шукаємо людину (клас 0)
    results = model.predict(frame, classes=[0], verbose=False)
    
    person_detected = False

    for result in results:
        # Якщо в кадрі є хоча б один об'єкт (людина)
    ...
    if person_detected and not is_armed:
        # Людина з'явилася -> відправляємо ARM
        try:
            requests.get(f"{ESP_CMD_URL}?arm=1", timeout=0.2)
            is_armed = True
            print("Людину знайдено! Дрон ЗААРМЛЕНО.")
        except requests.exceptions.RequestException:
            print("Помилка зв'язку з ESP32!")
            
    elif not person_detected and is_armed:
        # Людина зникла -> відправляємо DISARM
        try:
